...
class ModelMetrics:
    ...
    def record_request(self, model: str, success: bool, response_time: float, is_fallback: bool = False):
        with self.lock:
            key = f"{model}"
            self.requests[key] += 1
    ...
    def get_stats(self):
        with self.lock:
            return {
                ...
                "failure_rate": sum(self.failures.values()) / max(sum(self.requests.values()), 1),
                "fallback_rate": sum(self.fallbacks.values()) / max(sum(self.requests.values()), 1),
                "avg_response_times": {
